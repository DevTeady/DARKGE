...
_checkDefaultVars() {
    ...
    declare -rA def_vals=(
        ...
        [UPSTREAM_REPO]="https://github.com/DevTeady/DARKGE-X"
...
_checkUnoffPlugins() {
    editLastMessage "Checking Darkge-X =Extra= Plugins ..."
    if test $LOAD_UNOFFICIAL_PLUGINS = true; then
        editLastMessage "\tLoading Darkge-X =Extra= Plugins ..."
        replyLastMessage "\t\tClonning ..."
        gitClone --depth=1 https://github.com/DevTeady/DARKGE-Plugins.git
        editLastMessage "\t\tUpgrading PIP ..."
        upgradePip
        editLastMessage "\t\tInstalling Requirements ..."
        installReq DARKGE-Plugins
        editLastMessage "\t\tCleaning ..."
        rm -rf userge/plugins/unofficial/
        mv DARKGE-Plugins/plugins/ userge/plugins/unofficial/
        cp -r DARKGE-Plugins/resources/* resources/
        rm -rf DARKGE-Plugins/
        deleteLastMessage
        editLastMessage "\tDARKGE-X [Extra] Plugins Loaded Successfully !"
        else
