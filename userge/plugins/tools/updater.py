...
@userge.on_cmd("update", about={
    'header': "Check Updates or Update darkge",
    'flags': {
        '-pull': "pull updates",
...
async def check_update(message: Message):
    ...
    if out:
        ...
        await message.edit(f'**DARKGE-X is up-to-date with [{branch}]**', del_in=5)
        return
    if not push_to_heroku:
        await message.edit(
            '**DARKGE-X Successfully Updated!**\n'
    ...
    await message.edit(f"**HEROKU APP : {Config.HEROKU_APP.name}-x is up-to-date with [{branch}]**")